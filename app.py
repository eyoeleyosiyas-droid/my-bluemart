...
def get_product_owner(product_name):
    """Helper: look up who owns a listing, for ownership checks in routes."""
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT seller_username FROM products WHERE TRIM(LOWER(product_name)) = TRIM(LOWER(%s));", (product_name,))
    row = cur.fetchone()
    cur.close()
    conn.close()
